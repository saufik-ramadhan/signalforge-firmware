...
char **SDTools::readChunk(File &file, char **chunk, char *format, size_t maxLine, size_t maxLen)
{
  char lineBuffer[maxLen];
  size_t i = 0; // chunk counter
  while (i < maxLine - 1)
  {
    readLine(file, lineBuffer, maxLen);
  }
}

void SDTools::listFileInDir (File dir)
{
  while (true)
  {
