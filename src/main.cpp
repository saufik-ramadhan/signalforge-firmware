...
void buttonMenuTask(void *parameter)
{
    ...
    while (true)
    {
        ...
        if (xQueueReceive(buttonEventQueue, &event, portMAX_DELAY) == pdPASS)
        {
            ...
            {
            case BUTTON_UP:
                menu.navigateUp(currentMenuState);
                break;
            case BUTTON_DOWN:
                menu.navigateDown(currentMenuState);
                break;
            case BUTTON_RIGHT:
    ...
    while (1)
    {
        ...
        case INFRARED_MENU_READING_DONE_SAVING:
            ...
            else
            {
                ...
                csvFile = SD.open("/ircommand.csv", FILE_WRITE);
                if (csvFile.print("name;command;address;protocol;action\n"))
                {
                ...
                for (int i = 0; i < irDataCount; i++) // print current + additional data
                {
...
void setup()
{
    ...
    xTaskCreatePinnedToCore(buttonMenuTask, "Button Menu Task", 2048, NULL, 1, NULL, app_cpu);
